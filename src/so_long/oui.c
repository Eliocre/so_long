...
// 		ft_lstiter_param(game->player.anim.animright, update_anim, game);
// 	if (game->player.player_stats.lastmove == 1)
// 		ft_lstiter_param(game->player.anim.animright, update_anim, game);
// 	if (game->player.player_stats.lastmove == 1)
// 		ft_lstiter_param(game->player.anim.animright, update_anim, game);
...
void	update_map(t_game *game, t_coord next)
{
	...
	{
		game->map[game->player.player_coord.y][game->player.player_coord.x] = game->player.player_stats.curspot;
		game->player.player_stats.curspot = game->map[next.y][next.x];
	}
	else
	{
		game->map[game->player.player_coord.y][game->player.player_coord.x] = '0';
		game->player.player_stats.curspot = game->map[game->player.player_coord.y][game->player.player_coord.x];
...
{
	if (game->map[game->player.player_coord.y][game->player.player_coord.x - 1] == '1'
	&& dir == 1)
		return (1);
	if (game->map[game->player.player_coord.y - 1][game->player.player_coord.x] == '1'
	&& dir == 2)
		return (1);
	if (game->map[game->player.player_coord.y][game->player.player_coord.x + 1] == '1'
	&& dir == 3)
...
int	ft_event(int keysym, t_game *game)
{
	...
	{//to make into a separate func
		if (is_legal(game, 1) == 0)
		{
			...
			update_map(game, (t_coord){game->player.player_coord.x - 1, game->player.player_coord.y});
	...
	if (keysym == XK_w)
	{
		...
		{
			mlx_clear_window(game->win.mlx, game->win.win);
			update_map(game, (t_coord){game->player.player_coord.x, game->player.player_coord.y - 1});
			loadmap(game->map, game);
			// tmp = ft_itoa(game->player.player_stats.movecmp);
...
void	generate_anim_up(t_game *game, t_player *player)
{
	t_player	*temp;

//!(game->n_frames % (((game->panic_mode * 7 + game->g_rate) + game->g_rate) / 7)
	temp = player;
	if ()
		temp->sprites.animup = temp->sprites.animup->next;
	// if (!temp->sprites.up)
	// 	temp->sprites.up = temp->sprites.up_bak;
	mlx_put_image_to_window(game->win.mlx, game->win.win, temp->sprites.animup->content, 
		temp->player_coord.x, temp->player_coord.y);
}
